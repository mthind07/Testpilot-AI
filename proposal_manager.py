#Saves AI repair proposals, shows proposed changes as unified diffs, applies changes only after human approval
#Creates backups before editing supports rollback
...
#can only modify controlled demo-code locations
EDITABLE_TOP_LEVEL_DIRECTORIES = {
    "sample_app",
    "tests",
}

#protect TestPilot's own infrastructure and safety tests
PROTECTED_FILES = {
...
def _proposal_path(
    ...
    """Return a safe path for one proposal JSON file."""
...
def _validate_target(
    ...
    """Validate that a proposed file can safely be changed."""
    ...
    if relative_path.parts[0] not in EDITABLE_TOP_LEVEL_DIRECTORIES:
        raise ValueError(
...
def _build_updated_files(
    ...
    """Validate all changes and construct updated file contents."""
...
def _render_diff(
    ...
    """Create a readable Git-style diff for human review."""
...
def _atomic_write(target: Path, content: str) -> None:
    """Replace a file using a temporary file."""
    ...
    try:
        ...
        #replace only after the temporary file is complete
...
def get_proposal_diff(
    ...
    """Return the saved human-readable diff."""
...
def apply_proposal(
    ...
    """Apply a pending proposal after approval."""
    ...
    #revalidate against the current files
    updates = _build_updated_files(
        plan,
        project_root,
    )
    ...
    #back up every affected file before changing anything
    ...
    try:
        ...
        #restore the backups if writing fails partway through
        for target in updates:
            relative_path = target.relative_to(project_root)
            backup_path = backup_root / relative_path
